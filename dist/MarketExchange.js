...
export class MarketExchange {
    orderBooks = new Map(); // per ware
    priceHistory = new Map();
    marketStats = new Map();
    tradeHistory = [];
    nextOrderId = 1;
    stations = new Map(); // Station references for trade execution
    lastCleanup = 0;
    cleanupInterval = 5000; // Clean up expired orders every 5 seconds
    constructor() {
    ...
    executeTradeWithStations(execution) {
    ...
    updatePriceHistory(ware, price, volume) {
    ...
    updateMarketStats(ware) {
        const orderBook = this.orderBooks.get(ware);
        const history = this.priceHistory.get(ware);
        const stats = this.marketStats.get(ware);
        if (!orderBook || !history || !stats)
            return;
    ...
    getSupplyDemandRatio(ware) {
        const stats = this.marketStats.get(ware);
        if (!stats || stats.demandLevel === 0)
            return 1;
...
//# sourceMappingURL=MarketExchange.js.map