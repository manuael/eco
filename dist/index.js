...
class Order {
    ...
    qty;
    price;
    constructor(id, stationId, ware, side, qty, price) {
        this.id = id;
...
function initializeGame() {
    world = new World();
    // Create recipes
    const ironToSteel = new Recipe('iron_to_steel', [{ ware: 'iron', qty: 2 }], [{ ware: 'steel', qty: 1 }], 3, 1);
    const steelToShips = new Recipe('steel_to_ships', [{ ware: 'steel', qty: 5 }], [{ ware: 'ships', qty: 1 }], 5, 2);
    // Create stations
    ...
    // Create ships
    world.ships.push(new Ship('trader1', 'TRADER', [150, 150]), new Ship('miner1', 'MINER', [120, 120]), new Ship('trader2', 'TRADER', [400, 300]));
    // Create asteroid belts
    world.asteroidBelts.push(new AsteroidBelt('belt1', 'iron', [600, 100], 500), new AsteroidBelt('belt2', 'copper', [700, 450], 300));
}
// Rendering
function render() {
    if (!canvas || !ctx)
