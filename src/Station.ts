...
export class Station {
    ...
    public orders: Order[];
    public marketConfig: StationMarketConfig;
    private market: MarketExchange | null = null;
    public lastMarketUpdate: number = 0;
    public marketUpdateInterval: number = 8000; // 8 seconds
    private priceMemory: Map<string, PriceMemory> = new Map();
    ...
    private initializePriceMemory(ware: string): void {
    ...
    private handleProduction(deltaTime: number): void {
    ...
    private updateMarketOrders(): void {
            ...
            recipe.inputs.forEach(input => {
                ...
                if (currentStock < threshold) {
                    ...
                    const quantity = Math.min(
                        this.marketConfig.maxOrderSize,
                        Math.ceil((threshold - currentStock) * 1.5)
                    );

                    if (quantity > 0 && this.credits >= buyPrice * quantity) {
                        const order = new Order(
                            `${this.id}_buy_${input.ware}_${Date.now()}`,
                            this.id,
                            input.ware,
                            'BUY',
                            quantity,
                            buyPrice,
                            this.marketConfig.orderLifetime
                        );
        ...
        Object.keys(this.stock).forEach(ware => {
            ...
            if (currentStock > threshold) {
                ...
                const quantity = Math.min(
                    this.marketConfig.maxOrderSize,
                    Math.floor(currentStock - threshold)
                );

                if (quantity > 0) {
                    const order = new Order(
                        `${this.id}_sell_${ware}_${Date.now()}`,
                        this.id,
                        ware,
                        'SELL',
                        quantity,
    ...
    private calculateAdaptiveBuyPrice(ware: string): number {
    ...
    private calculateAdaptiveSellPrice(ware: string): number {
        const memory = this.priceMemory.get(ware)!;
        const marketPrice = this.market!.getCurrentPrice(ware);
    ...
    private getProductionCost(ware: string): number {
